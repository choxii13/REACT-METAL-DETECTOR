...
const MyBoxArgument = ({ x, y, id, drag, className, imagelink }) => {
  ...
  useEffect(() => {
    if (x > 5 && x < 20 && y > 25 && y < 50 && !drag && className === "metal") {
      setDesignMetal(style.add), setSensor("metal"), setMetalSave(true);
    } else if (
      x > 5 &&
      x < 20 &&
      y > 25 &&
      ...
      setDesignMetal(style.addred),
        setSensor("metal-wrong"),
    ...
    if (
      ...
      setDesignNonMetal(style.add), setSensor("non-metal");
      ...
      setDesignNonMetal(style.addred),
        setSensor("non-metal-wrong"),
        setMetalSave(false);
    } else {
      setDesignNonMetal(null);
      setNotMetalSave(false);
    }
